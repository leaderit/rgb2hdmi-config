...
NAME=$(sed -r -n 's/^[ ]*name[ ]*=[ ]*"(.*)"/\1/p' Cargo.toml)
VER=$(sed -r -n 's/^[ ]*version[ ]*=[ ]*"(.*)"/\1/p' Cargo.toml)
...
lipo -create -output $NAME-$VER-macos target/$ARC_ARM64/release/$NAME target/$ARC_X86/release/$NAME

cargo build --release --target=$ARC_WIN64 
mv target/$ARC_WIN64/release/$NAME.exe $NAME-$VER-win-x86_64.exe

# needs docker have started
cross build --release --target=$ARC_LINUX64
mv target/$ARC_LINUX64/release/$NAME $NAME-$VER-linux-x86_64

# Sign mac Os binary
codesign -s "Valerii Grazhdankin" $NAME-$VER-macos
